...
class MetadataQuery(spark : SparkSession, config: CluesoConfig, bucketName : String, sqlWhereExpr : String, start : Int, end : Int) {
  ...
  def execute() = {
    System.err.println("Query Execution – Spark Conf:")
    spark.conf.getAll.foreach { case (k,v) =>
      System.err.println(s"${k} = ${v}")
    }

    System.err.println("Query Execution – Clueso Conf:")
...
class MetadataQueryExecutor(spark : SparkSession, config : CluesoConfig) {

  var bucketDfs = Map[String, AtomicReference[DataFrame]]()


//  def execute(query : MetadataQuery) = {
//    if (!bucketDfs.contains(query.bucketName)) {
...
//      import SparkUtils.fillNonExistingColumns
//
////      bucketDfs = bucketDfs = landingTable.select(fillNonExistingColumns(colsLanding, unionCols):_*)
////        .union(stagingTable.select(fillNonExistingColumns(colsStaging, unionCols):_*))
////        .orderBy(col("key"))
////        .withColumn("rank", dense_rank().over(windowSpec))
////        .where(col("rank").lt(2).and(col("type") =!= "delete"))
...
//    val queryExec = new MetadataQueryExecutor(spark, config)
    ...
    query = Some(new MetadataQuery(spark, config, bucketName, sqlWhereExpr, start = 0, end = 1000))
    query.get.execute()
